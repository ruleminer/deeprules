...
def simplify_ruleset(ruleset: AbstractRuleSet):
    ruleset = deepcopy(ruleset)
    for rule in ruleset.rules:
        rule.premise = _simplify_condition(rule.premise)
    return ruleset


def _merge_intervals(condition: CompoundCondition):
    if (
        not isinstance(condition, CompoundCondition) or
        condition.logic_operator != LogicOperators.CONJUNCTION
    ):
        return
    ...
    for subcondition in condition.subconditions:
        if (
            isinstance(subcondition, ElementaryCondition) and
            (subcondition.left == float('-inf')
             or subcondition.right == float('inf'))
        ):
...
def _simplify_condition(condition: AbstractCondition) -> AbstractCondition:
    ...
    if condition.negated and all(not isinstance(s, CompoundCondition) for s in condition.subconditions):
    ...
    for s in condition.subconditions:
        if (
            isinstance(s, CompoundCondition) and
            s.logic_operator == condition.logic_operator and
            not s.negated
        ):
            tmp.append(s)
    for e in tmp:

...
def get_condition_weight(condition: AbstractCondition) -> float:
    ...
    if isinstance(condition, AttributesRelationCondition) or isinstance(
...
def get_more_preferable_condition(
    c1: AbstractCondition, c2: AbstractCondition
) -> AbstractCondition:
    ...
    if c2 is None and c1 is not None:
        return c1
    return max(
        [(c1, get_condition_weight(c1)), (c2, get_condition_weight(c2))],
        key=lambda x: x[1],
    )[0]
